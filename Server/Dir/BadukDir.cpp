// BadukDir.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CBadukDirApp, CWinApp)
	//{{AFX_MSG_MAP(CBadukDirApp)
...
/////////////////////////////////////////////////////////////////////////////
// CBadukDirApp construction

CBadukDirApp::CBadukDirApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CBadukDirApp object

CBadukDirApp theApp;
TCHAR g_szAppName[256];

/////////////////////////////////////////////////////////////////////////////
// CBadukDirApp initialization

BOOL CBadukDirApp::InitInstance()
{
	TCHAR szOption[32];
...
//		m_strPassword = _T("rudnfdldirl");
	...
	_splitpath(szExeFileName,szDrive,szDir,szFname,szExt);
	m_strPath.Format (_T("%s%s"), szDrive, szDir);
#endif//_DEBUG
}
...
//	g_ErrorLog.WriteError2(TRUE, _T(""),_T("OpenServer가 호출,m_sockListen"));
	m_fSuspend = FALSE; 
	...
	if (::select (0, &fdr, NULL, NULL, &t))
	{
		...
		if (FD_ISSET (m_sockListen, &fdr))
		{
			...
			else
			{
				...
				if (pDes->Create (m_sockListen))
				{
//					g_ErrorLog.WriteError2(TRUE, _T(""),_T("OnOpenSession를 열려고 합니다"));

					if (!OnOpenSession (pDes))
					{
//						g_ErrorLog.WriteError2(TRUE, _T(""),_T("OnOpenSession 열기 실패"));
